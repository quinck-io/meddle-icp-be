use crate::{
    common::{
        structures::{Data, JsonInput, OperationResult},
        uuid::uuid_v4,
    },
...
pub fn post_data(vec_json_data: Vec<JsonInput>) -> OperationResult {
    ...
    vec_json_data.iter().for_each(|x| {
        let unit_id = uuid_v4(local_timestamp);
        ids.push(unit_id.clone());

        x.variables.iter().for_each(|input| {
    ...
    if code == 400 {
        return OperationResult {
            unit_id: ids,
            code,
            message: String::from("Error in writing elements in database"),
        };
    }
    OperationResult {
        unit_id: ids,
        code,
        message: String::from("All Element Inserted Correctly"),
    }
}
